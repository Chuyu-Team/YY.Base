...
namespace YY
{
    namespace Base
    {
        namespace IO
        {
            ...
            uStringView __YYAPI Path::GetPathRoot(const uStringView& _sPath)
            {
            ...
            bool __YYAPI Path::IsPathRooted(const uStringView& _sPath)
            {
                return GetPathRoot(_sPath).GetLength() != 0;
            }

            bool __YYAPI Path::RemoveFileSpec(uStringView& _sPath)
            {
                if(_sPath.IsEmpty())
                    return false;
            ...
            uStringView __YYAPI Path::GetFileName(const uStringView& _sPath)
            {
                if(_sPath.IsEmpty())
                    return uStringView();
            ...
            uStringView __YYAPI Path::GetExtension(const uStringView& _sPath)
            {
            ...
            uStringView __YYAPI Path::GetFileNameWithoutExtension(const uStringView& _sPath)
            {
                auto _sFileName = GetFileName(_sPath);
                const auto _uExtensionIndex = _sFileName.LastIndexOf('.');
                if (_uExtensionIndex != kuInvalidIndex)
            ...
            uStringView __YYAPI Path::GetDirectoryPath(uStringView _sPath)
            {
            ...
            bool __YYAPI Path::IsPathFullyQualified(const uStringView& _sPath)
            {
            ...
            bool __YYAPI Path::Append(uString& _szPath, const uStringView& _sPathAppend)
            {
                ...
                else
                {
                    ...
                    _szPath.LockBuffer(_szPath.GetLength() + 1 + _cchAppend);
            ...
            uString __YYAPI Path::GetCurrentDirectory()
            {
                ...
                else
                {
                    ...
                    for (;;)
                    {
                        const auto _uLength = ::GetCurrentDirectoryW(_cchCurrentDirectory, _szCurrentDirectory.LockBuffer(_cchCurrentDirectory));
